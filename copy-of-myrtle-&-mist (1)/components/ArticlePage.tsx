...
import { ARTICLES, MYRTLE_PRODUCTS, MIST_PRODUCTS } from '../constants';
...
const ArticlePage: React.FC<ArticlePageProps> = ({ articleId, onNavigate }) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    // Reset animations on article change
  ...
  // Generate TOC items when content changes
  useEffect(() => {
    // Small delay to ensure DOM is rendered with new HTML content
  ...
  // Logic to find related articles (using the title from active theme)
  ...
  // Logic to find related products for "Shop This Story"
  ...
  return (
    <div className={`min-h-screen pt-24 pb-20 theme-transition ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Breadcrumbs view="article" title={activeContent.title} onNavigate={(v) => onNavigate(v)} />
        ...
        <header className="mb-12 text-center max-w-4xl mx-auto relative z-10 animate-fade-in-up">
          ...
          <p className={`text-sm font-medium theme-transition ${
            ...
            {article.readTime} • By {theme === 'myrtle' ? 'Myrtle Team' : 'Mist Editorial'}
          </p>
        </header>
        ...
        >
          ...
          <div className={`absolute inset-0 z-0 theme-transition ${
             ...
             {theme === 'myrtle' ? (
                ...
                <>
                  <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-purple-900/30 via-indigo-900/30 to-black/50 animate-gradient-x"></div>
                  <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[100%] bg-mist-accent/10 rounded-full blur-3xl animate-blob"></div>
                </>
             )}
          </div>

          {/* Image Texture (Low Opacity Overlay) */}
          ...
          {/* Gradient Fade for Text Safety (Bottom) */}
        ...
        >
          ...
          {/* --- LEFT SIDEBAR: STICKY TOC (3 cols) --- */}
          ...
          <div className="lg:col-span-7">
            ...
            <article className={`prose prose-lg max-w-none theme-transition ${
               theme === 'myrtle' 
                 ? 'prose-headings:font-geo text-gray-700 prose-a:text-myrtle-accent hover:prose-a:text-myrtle-text prose-a:transition-colors prose-a:decoration-myrtle-accent/40 prose-a:underline-offset-4 active:prose-a:text-myrtle-text/80' 
                 : 'prose-invert text-gray-300 prose-a:text-mist-accent hover:prose-a:text-white prose-a:transition-colors prose-a:decoration-mist-accent/40 prose-a:underline-offset-4 active:prose-a:text-white/80'
            }`}>
               {/* Content Injection */}
               <div dangerouslySetInnerHTML={{ __html: activeContent.content }} />
            </article>

            {/* Theme-Specific Footer CTA */}
          ...
          <aside className="lg:col-span-3 space-y-8 relative">
            <div className="sticky top-32">
               ...
               {/* Shop This Story Micro-Interaction */}
               {shopThisStory.length > 0 && (
                 <div className={`p-6 rounded-xl border theme-transition ${
                   theme === 'myrtle' ? 'bg-white/80 border-gray-100 shadow-xl backdrop-blur-md' : 'bg-mist-secondary/80 border-mist-accent/10 shadow-2xl backdrop-blur-md'
                 }`}>
                   ...
                   <div className="space-y-6">
                     {shopThisStory.map(product => (
                       <div key={product.id} className="flex gap-4 items-start group cursor-pointer" onClick={() => window.open(product.affiliateUrl, '_blank')}>
                         ...
                         <div className="flex-grow pt-1">
                           <h4 className={`text-sm font-bold leading-tight mb-2 group-hover:text-opacity-80 transition-colors ${
                             theme === 'myrtle' ? 'text-myrtle-text' : 'text-white'
                           }`}>
                             {product.name}
                           </h4>
