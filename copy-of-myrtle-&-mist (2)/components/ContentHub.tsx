...
import { ARTICLES, THEME_CONTENT } from '../constants';
...
const ContentHub: React.FC<ContentHubProps> = ({ onNavigate }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {featuredArticles.map((article) => {
            ...
            return (
              <article 
                key={article.id} 
                className={`group cursor-pointer rounded-xl overflow-hidden border theme-transition transform hover:-translate-y-2 duration-500 ${
                  theme === 'myrtle' 
