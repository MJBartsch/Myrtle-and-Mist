...
import { LIGHT_ZONES } from '../constants';
import { ViewType } from '../types';

interface LightArchitectProps {
  onNavigate?: (view: ViewType, id?: string) => void;
}

type ObstructionType = 'none' | 'sheer' | 'trees' | 'blinds';

const LightArchitect: React.FC<LightArchitectProps> = ({ onNavigate }) => {
  const { theme } = useTheme();
  const [distance, setDistance] = useState(2); // Feet
  ...
  const windowIntensities = {
      'S': 2000, // South: Very Bright
      'W': 1500, // West: Bright afternoon
      'E': 1000, // East: Gentle morning
      'N': 500   // North: Low
  };
  ...
  const calculateLux = (dist: number, dir: 'N' | 'E' | 'S' | 'W', obs: ObstructionType) => {
    ...
    // Inverse Square Law with a "Window Size" buffer (0.5)
    // Intensity = (Source * Obstruction) / Distance^2
    const rawFc = (sourceIntensity * modifier) / Math.pow(Math.max(dist, 0.5), 2);
    
    // Cap at reasonable max (Direct Sun) and min
  ...
  const gaugeMax = 2000; // Cap visual gauge at 2000 (Bright Indirect/Direct crossover)
  ...
  const handlePlantClick = (view?: ViewType, id?: string) => {
    if (view && onNavigate) {
       onNavigate(view, id);
    }
  };
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {[
                          ...
                          { id: 'blinds', label: 'Blinds / Screens' }
                            ...
                            >
                                {obs.label}
                                {obstruction === obs.id && <div className={`w-2 h-2 rounded-full ${theme === 'myrtle' ? 'bg-myrtle-accent' : 'bg-mist-accent'}`}></div>}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div className={`lg:col-span-7 p-8 md:p-10 relative flex flex-col ${
                 ...
                 <div className="flex flex-col md:flex-row gap-8 mb-10 items-center md:items-start relative z-10">
                    ...
                    <div className="relative w-48 h-32 flex flex-col items-center justify-end shrink-0">
                        ...
                        {/* Text Overlay - Positioned relative to bottom of container, not absolute centered */}
                 ...
                 <div className={`w-full h-2 rounded-full mb-8 relative overflow-hidden mt-6 ${
                     ...
                     {/* Zone Markers */}
                     <div className="absolute top-0 bottom-0 left-[5%] w-px bg-current opacity-20"></div> {/* Low End */}
                     <div className="absolute top-0 bottom-0 left-[25%] w-px bg-current opacity-20"></div> {/* Med */}
                     <div className="absolute top-0 bottom-0 left-[50%] w-px bg-current opacity-20"></div> {/* Bright */}
                 </div>
                 ...
                 {/* Bottom Row: Plant Recommendations */}
                 <div>
                     ...
                     <div className="grid grid-cols-2 gap-3">
                         {currentZone.plants.map((plant, idx) => {
                             const isLink = !!plant.linkId;
                             return (
                                 <button
                                     key={idx} 
                                     onClick={() => isLink && handlePlantClick(plant.linkView, plant.linkId)}
                                     disabled={!isLink}
                                 ...
                                 >
                                     ...
                                     {isLink && (
                                        <svg className={`w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity transform group-hover:translate-x-1 ${
                                            theme === 'myrtle' ? 'text-myrtle-accent' : 'text-mist-accent'
