...
import { NAV_LINKS, PILLAR_CONTENT, ARTICLES } from '../constants';
...
const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
  ...
  const getHeaderClasses = () => {
    ...
    // Solid/Glass style on scroll or when menu is active
    return theme === 'myrtle' 
  ...
  const getLinkClasses = (linkName: string) => {
    const isActive = activeDropdown === linkName;
    ...
    if (!isScrolled && !activeDropdown && !isMobileMenuOpen) {
       colorClass = theme === 'myrtle' ? 'text-myrtle-text/80 hover:text-myrtle-accent' : 'text-mist-text/80 hover:text-mist-accent';
    } else {
       colorClass = theme === 'myrtle' ? 'text-myrtle-text hover:text-myrtle-accent' : 'text-mist-text hover:text-mist-accent';
    }
    ...
    if (activeDropdown === 'guide') {
       ...
       return (
         <div className="grid grid-cols-12 gap-8">
                ...
                {categories.map((cat) => (
                   <button 
                     ...
                     className={`text-left p-3 rounded-lg flex items-start gap-4 transition-all hover:-translate-y-1 ${
                         theme === 'myrtle' 
                            ? 'hover:bg-green-50/50 hover:shadow-sm' 
                            : 'hover:bg-white/5'
                     }`}
                   >
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                         theme === 'myrtle' ? 'bg-white text-myrtle-accent shadow-sm' : 'bg-white/10 text-mist-accent'
                          ...
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={cat.icon} /></svg>
                      </div>
                      <div>
                          <h4 className={`text-sm font-bold mb-1 ${theme === 'myrtle' ? 'text-myrtle-text' : 'text-white'}`}>{cat.title}</h4>
                          <p className={`text-xs ${theme === 'myrtle' ? 'text-gray-500' : 'text-gray-400'}`}>{cat.description}</p>
                      </div>
                   </button>
                ))}
            </div>
            ...
            {/* Featured Side */}
            <div className={`col-span-4 p-6 rounded-xl flex flex-col justify-end relative overflow-hidden group cursor-pointer ${
                    ...
                    return (
                        ...
                        >
                            <div className="aspect-video rounded-lg overflow-hidden mb-3 relative">
                                <img src={article.image} alt={content.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center relative">
        ...
        <div className="flex-shrink-0 z-50">
          <button onClick={() => onNavigate('home')} className={`focus:outline-none theme-transition ${
            isScrolled 
      ...
      <div className={`md:hidden fixed inset-0 z-40 transition-all duration-300 ${
         ...
         <div className={`absolute inset-0 ${
             theme === 'myrtle' ? 'bg-myrtle-bg/98' : 'bg-mist-bg/98'
         }`}></div>
         ...
         <div className="relative z-50 h-full flex flex-col pt-24 px-6 overflow-y-auto pb-10">
            {NAV_LINKS.map((link) => {
               ...
               return (
                   <div key={link.name} className={`border-b ${theme === 'myrtle' ? 'border-gray-200' : 'border-white/10'}`}>
                       ...
                       {hasSubmenu && (
                           <div className={`overflow-hidden transition-all duration-300 ${isExpanded ? 'max-h-[500px] opacity-100 mb-4' : 'max-h-0 opacity-0'}`}>
                               <div className="pl-4 space-y-3">
                                   ...
                                   <button 
                                      onClick={() => { onNavigate('pillar'); setIsMobileMenuOpen(false); }}
                                      className={`block text-sm font-bold mt-4 uppercase ${theme === 'myrtle' ? 'text-myrtle-accent' : 'text-mist-accent'}`}
                                   >
                                       View All Guides
                                   </button>
                               </div>
